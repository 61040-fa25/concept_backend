...
Deno.test("SessionConcept functionality", async (t) => {
  ...
  await t.step(
    "changeSession: should initialize a new session correctly",
    async () => {
  ...
  await t.step(
    "changeSession: should prevent changing session if current session is active",
    async () => {
      ...
      assertEquals(result, {
        error:
          `User '${USER_ALICE}' already has an active session. Please end it first.`,
      });
  ...
  await t.step(
    "changeSession: should allow changing session if current session is inactive",
    async () => {
      // End the active session first
      await sessionConcept.endSession({ sessionOwner: USER_ALICE });
      const inactiveSession = await sessionConcept._getSession({
        sessionOwner: USER_ALICE,
      });
  ...
  await t.step(
    "setOrdering: should set ordering for an inactive session",
    async () => {
  ...
  await t.step(
    "setOrdering: should not set ordering for an active session",
    async () => {
      // Activate session
      await sessionConcept.activateSession({ activator: USER_ALICE });
      ...
      assertEquals(result, {
        error:
          `Cannot change ordering while session for '${USER_ALICE}' is active.`,
      });
  ...
  await t.step(
    "setFormat: should set format for an inactive session",
    async () => {
      // Ensure session is inactive
      await sessionConcept.endSession({ sessionOwner: USER_ALICE });
  ...
  await t.step(
    "setFormat: should not set format for an active session",
    async () => {
      // Activate session
      await sessionConcept.activateSession({ activator: USER_ALICE });
      ...
      assertEquals(result, {
        error:
          `Cannot change format while session for '${USER_ALICE}' is active.`,
      });
  ...
  await t.step(
    "randomizeOrder: should randomize order when ordering is 'Random'",
    async () => {
      ...
      let sessionBefore = await sessionConcept._getSession({
        sessionOwner: USER_ALICE,
      });
      const initialRandomOrders = sessionBefore!.list.items.map((item) =>
        item.randomOrder
      );
      const initialDefaultOrders = sessionBefore!.list.items.map((item) =>
        item.defaultOrder
      );
      ...
      const sessionAfter = await sessionConcept._getSession({
        sessionOwner: USER_ALICE,
      });
      assertExists(sessionAfter);
      const newRandomOrders = sessionAfter.list.items.map((item) =>
        item.randomOrder
      );
      ...
      assertEquals(
        sortedNewRandomOrders,
        Array.from({ length: WORK_ITEMS.length }, (_, i) => i),
      );
    },
  );

  await t.step(
    "randomizeOrder: should not randomize order when ordering is not 'Random'",
    async () => {
      ...
      const sessionBefore = await sessionConcept._getSession({
        sessionOwner: USER_ALICE,
      });
      ...
      assertEquals(result, {
        error:
          `Cannot randomize order; session for '${USER_ALICE}' is not set to random ordering.`,
      });
  ...
  await t.step(
    "activateSession: should activate an inactive session",
    async () => {
      // Ensure session is inactive
      await sessionConcept.endSession({ sessionOwner: USER_ALICE });
  ...
  await t.step(
    "activateSession: should not activate an already active session",
    async () => {
      ...
      assertEquals(result, {
        error: `Session for '${USER_ALICE}' is already active.`,
      });
  ...
  await t.step("startTask: should start an 'Incomplete' task", async () => {
    ...
    const taskReport = session?.list.items.find((item) =>
      item.task === TASK_REPORT
    );
    assertEquals(taskReport?.itemStatus, TaskStatus.InProgress);

    const taskMeeting = session?.list.items.find((item) =>
      item.task === TASK_MEETING
    );
    assertEquals(taskMeeting?.itemStatus, TaskStatus.Incomplete); // Other task should remain Incomplete
  });

  await t.step(
    "startTask: should not start a task if another is 'In Progress'",
    async () => {
      ...
      assertEquals(
        sessionBefore?.list.items.find((item) => item.task === TASK_REPORT)
          ?.itemStatus,
      ...
      assertEquals(result, {
        error: `Another task ('${TASK_REPORT}') is already 'In Progress'.`,
      });

      const sessionAfter = await sessionConcept._getSession({
        sessionOwner: USER_ALICE,
      });
      const taskMeeting = sessionAfter?.list.items.find((item) =>
        item.task === TASK_MEETING
      );
  ...
  await t.step(
    "startTask: should not start a task that is not 'Incomplete'",
    async () => {
      ...
      const sessionAfterCompletion = await sessionConcept._getSession({
        sessionOwner: USER_ALICE,
      });
      assertEquals(
        sessionAfterCompletion?.list.items.find((item) =>
          item.task === TASK_REPORT
        )?.itemStatus,
        TaskStatus.Complete,
      );
      ...
      assertEquals(result, {
        error:
          `Task '${TASK_REPORT}' is not in 'Incomplete' status. Current: ${TaskStatus.Complete}.`,
      });
    },
  );

  await t.step(
    "startTask: should not start a task if session is inactive",
    async () => {
      await sessionConcept.endSession({ sessionOwner: USER_ALICE });
      const session = await sessionConcept._getSession({
        sessionOwner: USER_ALICE,
      ...
      assertEquals(result, {
        error:
          `Session for user '${USER_ALICE}' is not active. Activate it first.`,
      });
    },
  );

  await t.step(
    "completeTask: should complete an 'In Progress' task",
    async () => {
      ...
      assertEquals(
        sessionBefore?.list.items.find((item) => item.task === TASK_MEETING)
          ?.itemStatus,
        TaskStatus.InProgress,
      );
  ...
  await t.step(
    "completeTask: should not complete a task that is not 'In Progress'",
    async () => {
      ...
      assertEquals(result, {
        error:
          `Task '${TASK_REPORT}' is not in 'In Progress' status. Current: ${TaskStatus.Complete}.`,
      });
    },
  );

  await t.step(
    "completeTask: should not complete a task if session is inactive",
    async () => {
      await sessionConcept.endSession({ sessionOwner: USER_ALICE });
      const session = await sessionConcept._getSession({
        sessionOwner: USER_ALICE,
      ...
      const milkTask =
        (await sessionConcept._getSession({ sessionOwner: USER_ALICE }))?.list
          .items.find((i) => i.task === TASK_MILK);
  ...
  await t.step(
    "endSession: should not end an already inactive session",
    async () => {
      ...
      assertEquals(result, {
        error: `Session for '${USER_ALICE}' is not active.`,
      });
  ...
  await t.step(
    "Queries: _getSession should return null for non-existent session",
    async () => {
      const session = await sessionConcept._getSession({
  ...
  await t.step(
    "Queries: _getTasksInOrder should return tasks in default order",
    async () => {
      await sessionConcept.changeSession({
        listId: LIST_GROCERIES,
        listTitle: "Groceries",
      ...
      const orderedTasks = await sessionConcept._getTasksInOrder({
        sessionOwner: USER_BOB,
      });
  ...
  await t.step(
    "Queries: _getTasksInOrder should return tasks in random order",
    async () => {
      ...
      const expectedTasks = session!.list.items.sort((a, b) =>
      ...
      const defaultOrders = (orderedTasks as Array<
        { task: Task; order: number; status: TaskStatus }
      >).map((t) => t.order);
      const initialDefaultOrders = GROCERIES_ITEMS.map((item) =>
        item.defaultOrder
      );
  ...
  await t.step(
    "Principle Trace: a user will 'activate' a list to start a session and be given an ordered list (either default ordering or generated) of tasks on the list to complete",
    async () => {
      ...
      let result: Empty | { error: string } = await sessionConcept
        .changeSession({
          listId: list,
      ...
      let orderedTasks = await sessionConcept._getTasksInOrder({
        sessionOwner: user,
      });
      assertEquals(orderedTasks, [
      ...
      result = await sessionConcept.startTask({
        task: taskA,
        sessionOwner: user,
      });
      assertEquals(result, {});
      session = await sessionConcept._getSession({ sessionOwner: user });
      assertEquals(
        session?.list.items.find((i) => i.task === taskA)?.itemStatus,
        TaskStatus.InProgress,
      );

      result = await sessionConcept.completeTask({
        task: taskA,
        sessionOwner: user,
      });
      assertEquals(result, {});
      ...
      orderedTasks = await sessionConcept._getTasksInOrder({
        sessionOwner: user,
      });
      assertEquals(orderedTasks, [
