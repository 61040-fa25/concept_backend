...
function extractNotificationIds(
  result: Array<{ notifications: Notification[] }>,
): Notification[] {
...
Deno.test("NotificationConcept: Operational Principle Trace - Remind users to save and celebrate milestones", async () => {
  ...
  console.log(
    `Scenario: User ${aliceUserId} has a savings plan ${aliceSavingsPlanId}.`,
  );

  // Initial state check: Alice should have no notifications
  let aliceNotifications = extractNotificationIds(
    await concept._getAllNotifications({ user: aliceUserId }),
  );
  assertEquals(
    aliceNotifications.length,
    0,
    "Initial state: Alice should have 0 notification IDs.",
  );
  console.log("Initial State: Confirmed Alice has no notifications.");

  // Step 1: Create a weekly savings reminder notification
  console.log(
    `Action: An external system triggers creation of a weekly reminder notification for ${aliceUserId}.`,
  );
  ...
  assertExists(
    (createReminderResult as { notification: Notification }).notification,
    "Reminder notification ID should be returned.",
  );
  const reminderNotificationId =
    (createReminderResult as { notification: Notification }).notification;
  console.log(
    `Effect: Notification ${reminderNotificationId} created to remind ${aliceUserId}.`,
  );

  // Verify state: Alice should now have 1 notification ID
  aliceNotifications = extractNotificationIds(
    await concept._getAllNotifications({ user: aliceUserId }),
  );
  assertEquals(
    aliceNotifications.length,
    1,
    "After reminder: Alice should have 1 notification ID.",
  );
  assertEquals(
    aliceNotifications[0],
    reminderNotificationId,
    "Confirmation: Reminder notification ID exists.",
  );
  console.log(
  ...
  console.log(
    `Action: An external system triggers creation of a milestone celebration notification for ${aliceUserId}.`,
  );
  ...
  assertExists(
    (createCelebrationResult as { notification: Notification }).notification,
    "Celebration notification ID should be returned.",
  );
  const celebrationNotificationId =
    (createCelebrationResult as { notification: Notification }).notification;
  console.log(
    `Effect: Notification ${celebrationNotificationId} created to celebrate ${aliceUserId}'s milestone.`,
  );
  ...
  assertEquals(
    aliceNotifications.length,
    2,
    "After celebration: Alice should have 2 notification IDs.",
  );
  console.log(
    `Confirmation: Alice has ${aliceNotifications.length} notification IDs:`,
  );
  ...
  assertEquals(
    aliceNotifications[0],
    reminderNotificationId,
    "First notification ID should be the reminder (oldest createdAt).",
  );
  assertEquals(
    aliceNotifications[1],
    celebrationNotificationId,
    "Second notification ID should be the celebration (newest createdAt).",
  );
  console.log(
  ...
  console.log(
    `Action: User ${aliceUserId} deletes the reminder notification ${reminderNotificationId}.`,
  );
  const deleteResult = await concept.deleteNotification({
    user: aliceUserId,
    notification: reminderNotificationId,
  });
  assertEquals(
    deleteResult,
    {},
    "Reminder notification should be deleted successfully.",
  );
  console.log(
    `Effect: Reminder notification ${reminderNotificationId} deleted.`,
  );
  ...
  assertEquals(
    aliceNotifications.length,
    1,
    "Final state: Alice should now have 1 notification ID.",
  );
  assertEquals(
    aliceNotifications[0],
    celebrationNotificationId,
...
Deno.test("NotificationConcept: Interesting Case 3 - deleteNotification failure scenarios", async () => {
  ...
  console.log(
    `Action: Attempting to delete non-existent notification ${nonExistentId} for ${USER_ALICE}.`,
  );
  let result = await concept.deleteNotification({
    user: USER_ALICE,
  ...
  const bobNotificationResult = await concept.createNotification({
    ...
    frequency: 10,
    message: "Bob's private note.",
  });
  const bobNotificationId =
    (bobNotificationResult as { notification: Notification }).notification;
  console.log(
    `Setup: Created notification ${bobNotificationId} for ${USER_BOB}.`,
  );
  console.log(
    `Action: Attempting to delete ${bobNotificationId} (Bob's) by ${USER_ALICE}.`,
  );
  result = await concept.deleteNotification({
    user: USER_ALICE,
    notification: bobNotificationId,
  });
  assertExists(
    (result as { error: string }).error,
    "Should return an error for deleting another user's notification.",
  );
  assertEquals(
    (result as { error: string }).error,
  ...
  assertEquals(
    extractNotificationIds(
      await concept._getAllNotifications({ user: USER_BOB }),
    ).length,
    1,
    "Bob's notification should still exist.",
  );
  ...
  const tempNotificationId =
    (tempNotificationResult as { notification: Notification }).notification;
  console.log(
    `Setup: Created temporary notification ${tempNotificationId} for ${USER_ALICE}.`,
  );
  await concept.deleteNotification({
    user: USER_ALICE,
  ...
  console.log(
    `Action: Attempting to delete ${tempNotificationId} a second time.`,
  );
  result = await concept.deleteNotification({
    user: USER_ALICE,
    notification: tempNotificationId,
  ...
  assertExists(
    (result as { error: string }).error,
    "Should return an error for deleting an already deleted notification.",
  );
  assertEquals(
    (result as { error: string }).error,
...
Deno.test("NotificationConcept: Interesting Case 4 - _getAllNotifications for empty user", async () => {
  ...
  console.log(
    `Action: Getting notifications for user ${nonExistentUser} with no existing notifications.`,
  );
  const result = await concept._getAllNotifications({ user: nonExistentUser });
  assertEquals(
    result[0].notifications.length,
    0,
    "Should return an empty array of IDs.",
  );
  console.log(
...
Deno.test("NotificationConcept: Query getNotificationMessageAndFreq returns details and enforces ownership", async () => {
  const [db, client] = await testDb();
  const concept = new NotificationConcept(db);

  console.log("\n--- Query: getNotificationMessageAndFreq ---");

  const message = "Track your savings progress.";
  const frequency = 14;
  const createResult = await concept.createNotification({
    user: USER_ALICE,
    progress: PROGRESS_TRACKING_SAVINGS,
    frequency,
    message,
  ...
  const notificationId =
    (createResult as { notification: Notification }).notification;

  const queryResult = await concept.getNotificationMessageAndFreq({
    user: USER_ALICE,
    notification: notificationId,
  });
  assertEquals(
    (queryResult as { message: string; frequency: number }).message,
    message,
  );
  assertEquals(
    (queryResult as { message: string; frequency: number }).frequency,
    frequency,
  );
  console.log(
    "Confirmation: Returned message and frequency match stored values.",
  );

  const wrongUserResult = await concept.getNotificationMessageAndFreq({
    user: USER_BOB,
    notification: notificationId,
  });
  assertEquals(
    (wrongUserResult as { error: string }).error,
    `Notification with ID '${notificationId}' does not belong to user '${USER_BOB}'.`,
    "Should return ownership error for incorrect user.",
  );
  console.log("Requirement failure case verified for incorrect user.");
...
Deno.test("NotificationConcept: Interesting Case 5 - _getAllNotifications with multiple sorted results", async () => {
  ...
  const createN1 = await concept.createNotification({
    user: USER_ALICE,
    progress: PROGRESS_TRACKING_SAVINGS,
    frequency: 7,
  ...
  const createN2 = await concept.createNotification({
    user: USER_ALICE,
    progress: PROGRESS_TRACKING_INVESTMENT,
    frequency: 30,
  ...
  await new Promise((resolve) => setTimeout(resolve, 10)); // Using setTimeout
  const createN3 = await concept.createNotification({
    user: USER_ALICE,
    progress: PROGRESS_TRACKING_SAVINGS,
    frequency: 1,
  ...
  console.log(
    `Setup: Created N1 (${n1Id}), N2 (${n2Id}), N3 (${n3Id}) for Alice.`,
  );

  // Create a notification for Bob to ensure it's not mixed with Alice's
  await concept.createNotification({
    user: USER_BOB,
    progress: PROGRESS_TRACKING_SAVINGS,
    frequency: 7,
  ...
  assertEquals(
    ...
    3,
    "Should return 3 notification IDs for Alice.",
  );
  assertEquals(
    notificationIds[0],
    n1Id,
    "First notification ID should be N1 (oldest).",
  );
  assertEquals(
    notificationIds[1],
    n2Id,
    "Second notification ID should be N2.",
  );
  assertEquals(
    notificationIds[2],
    n3Id,
    "Third notification ID should be N3 (newest).",
  );
  console.log(
  ...
  const bobNotifications = extractNotificationIds(
    await concept._getAllNotifications({ user: USER_BOB }),
  );
  assertEquals(
    bobNotifications.length,
    1,
    "Bob should have only his notification ID.",
  );
  assertNotEquals(
    bobNotifications[0],
    n1Id,
    "Bob's notification should not be Alice's.",
  );
  console.log(
