...
parent: '[[..\20251029_190030.dbb90da1.md]]'
content_id: 60193b8732a7ddd0d6666b189dd8585bbb53767d5c19f94cbdd24e1a00f56491
---
...
      if (typeof parsed.flight === "number") {

        costEstimate.flight = parsed.flight;
...
      if (typeof parsed.foodDaily === "number") {

        costEstimate.foodDaily = parsed.foodDaily;
...
          "LLM response 'flight' field is not a number:",
...
        parsed.foodDaily !== undefined && typeof parsed.foodDaily !== "number"
...
        error: `Failed to parse LLM response into expected JSON format: ${
...
    // Deduplication: if a travel plan with the exact same attributes already exists,

    // return its ID instead of creating a new one.

    const existing = await this.travelPlans.findOne({
...
      return { travelPlan: existing._id as TravelPlan };
...
    { travelPlan: TravelPlan; necessity: Necessity } | { error: string }
...
    // The design no longer requires checking a `locations` collection here.

    // Use the stored IDs as string representations for the prompt. Only the

    // necessity document still needs to be fetched.

    const necessityDoc = await this.necessities.findOne({
...
      return { error: "Referenced necessity data not found." };
...
    return plans.map((plan: { _id: TravelPlan }) => plan._id as TravelPlan);
