---
timestamp: 'Wed Oct 22 2025 18:28:21 GMT-0400 (Eastern Daylight Time)'
parent: '[[..\20251022_182821.599c0975.md]]'
content_id: ad45694a258924f09df1c84f49eaa53ca003e84a5bf27f68e7e3b6638366e3d5
---
...
Deno.test("Principle: User registers, confirms, and can authenticate", async () => {
  ...
  try {
    ...
    assertEquals(authenticatedUser, user, "Authenticated user ID should match.");
  } finally {
    await client.close();
  }
});
...
Deno.test("Action: confirm successfully confirms a user and cleans up token", async () => {
  ...
  try {
    ...
    // After confirmation
    const postConfirmUser = await userAuthConcept._getUserById({ userId: user });
...
Deno.test("Action: confirm fails with invalid token", async () => {
  ...
  try {
    ...
    assertEquals((result as { error: string }).error, "Invalid confirmation token.");
