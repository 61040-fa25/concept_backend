...
content_id: 7a85963fb2d13c845bfb42ff93a6ab9dacc89b54e9ea119953d110ab7b5c92ee
---
...
export default class PlannerConcept {
  ...
  async planDay({ user, tasks, busySlots }: { user: User; tasks: TaskWithDuration[]; busySlots: BusySlot[] }): Promise<{ firstTask?: Task } | { error: string }> {
    ...
    const now = new Date();
    // Use a fixed start time for predictability in tests, like the beginning of the day.
  ...
  // Public query to get scheduled tasks for testing purposes
  async _getScheduledTasksForUser({ user }: { user: User }) {
    return await this.scheduledTasks.find({ owner: user }, { sort: { plannedStart: 1 } }).toArray();
  }
